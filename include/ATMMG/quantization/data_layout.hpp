...
struct BatchDataMap {
   public:
    explicit BatchDataMap(char* data, size_t padded_dim)
        ...
        , f_add_(
              reinterpret_cast<T*>(data + (padded_dim * scan::kBatchSize / 8))
          )  // 1 bit code
        , f_rescale_(f_add_ + scan::kBatchSize)
        , f_error_(f_rescale_ + scan::kBatchSize) {}
    ...
    static size_t data_bytes(size_t padded_dim) {
        return (padded_dim * scan::kBatchSize / 8) +
               (sizeof(T) * scan::kBatchSize * 3);
    }
...
struct ConstBatchDataMap {
   public:
    explicit ConstBatchDataMap(const char* data, size_t padded_dim)
        ...
        , f_add_(
              reinterpret_cast<const T*>(data + (padded_dim * scan::kBatchSize / 8))
          )  // 1 bit code
        , f_rescale_(f_add_ + scan::kBatchSize)
        , f_error_(f_rescale_ + scan::kBatchSize) {}
