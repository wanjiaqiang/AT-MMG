...
#include "ATMMG/scan/scan.hpp"
#include "ATMMG/quantization/data_layout.hpp"
#include "ATMMG/quantization/scalar_quant_impl.hpp"

namespace ATMMG::quant {

struct QuantConfig {
    double t_const = -1;
    explicit QuantConfig() = default;
    QuantConfig(QuantConfig const&) = default;
    QuantConfig(QuantConfig&&) = default;
    QuantConfig& operator=(const QuantConfig&) = default;
    QuantConfig& operator=(QuantConfig&&) = default;
};

// init config for faster quantization ATMMG, for total_bits > 0, store const rescaling
// factor in QuantConfig
inline QuantConfig make_quant_config(size_t dim, size_t total_bits) {
    QuantConfig config;
    if (total_bits > 1) {
        config.t_const =
            scalar_impl::ex_bits::get_const_scaling_factors(dim, total_bits - 1);
    }
    return config;
}
...
inline void quantize_one_batch(
    ...
    scalar_impl::one_bit::one_bit_batch_code(
        data,
        centroid,
        num,
        padded_dim,
...
static inline void quantize_qg_batch(
    ...
    std::vector<T> f_error(scan::kBatchSize);  // we dont need this factor for qg
    QGBatchDataMap<T> cur_batch(batch_data, padded_dim);

    scalar_impl::one_bit::one_bit_batch_code<T>(
        data,
        centroid,
        num,
        padded_dim,
...
inline void quantize_compact_one_bit(
    ...
    scalar_impl::one_bit::one_bit_compact_code(
        data,
        centroid,
        padded_dim,
...
inline void quantize_compact_ex_bits(
    ...
    scalar_impl::ex_bits::ex_bits_compact_code(
        data,
        centroid,
        padded_dim,
        ex_bits,
...
inline void quantize_full_single(
    ...
    scalar_impl::total_bits::full_quant_impl<T, TP>(
        data,
        centroid.data(),
        dim,
        total_bits,
...
inline void quantize_scalar(
    ...
    std::vector<T> centroid(dim, 0);
    scalar_impl::total_bits::scalar_quant_impl<T, TP>(
        data,
        centroid.data(),
        dim,
        total_bits,
