...
#include "ATMMG/quantization/pack_code.hpp"
#include "ATMMG/utils/space.hpp"

namespace ATMMG::quant::scalar_impl {
...
inline void one_bit_batch_code(
    ...
    scan::pack_codes(padded_dim, compact_codes.data(), num, packed_code);
}
...
namespace ex_bits {
/**
 * @brief Get quantization code and factor (ipnorm_inv) for scalar quantization
 ...
 * @param code      Quantization code of scalar quantization, length of dim, stored as uint8
...
inline void ex_bits_compact_code(
    ...
    packing_extra_code(ex_code.data(), compact_code, padded_dim, ex_bits);
}
}  // namespace ex_bits

namespace total_bits {
template <typename T, typename TP>
static inline void scalar_quant_impl(
    const T* data,
    const T* centroid,
    size_t dim,
    size_t total_bits,
    ...
    RowMajorArray<T> residual_arr =
        scalar_impl::one_bit::one_bit_code(data, centroid, dim, binary_code.data());
...
template <typename T, typename TP>
static inline void full_quant_impl(
    const T* data,
    const T* centroid,
    size_t dim,
    size_t total_bits,
