...
#pragma omp parallel for schedule(dynamic)
    for (std::int64_t i = 0; i < static_cast<std::int64_t>(num_points); ++i) {
        size_t row = static_cast<size_t>(i);
        auto tid = omp_get_thread_num();
        std::vector<T>& cur_results = all_results[tid];
        const T* cur_data = data + (dim * row);
...
#pragma omp parallel for schedule(dynamic)
    for (std::int64_t i = 0; i < static_cast<std::int64_t>(num_points); ++i) {
        ...
        if (distance < cur_entry.distance) {
            cur_entry.id = static_cast<PID>(row);
...
static inline void transpose_binary_code(
