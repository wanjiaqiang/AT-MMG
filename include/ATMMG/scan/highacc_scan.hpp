...
namespace ATMMG::scan {
/**
 * @brief Change u16 lookup table to u8. Since we use more bits (higher accuracy)
 * to quantize data vector by scalar quantization, we also needs to increase the accuracy of data in
 * lut.
...
#else
        static_assert(false, "At least requried AVX2 for using scan\n");
        exit(1);
#endif
...
#else
    std::cerr << "AVX512 or AVX2 required for using scan\n";
    exit(1);
#endif
}
}  // namespace ATMMG::scan